import { IBlogPost } from "../../resources/elements/blog-card/blog-card";
...
export class PostService {
  ...
  private sortByDate(a: IBlogPost, b: IBlogPost) {
    return a.createdAt.getTime() - b.createdAt.getTime();
  }

  getAllPosts(): Promise<IBlogPost[]> {
  ...
  allPostPreviews(): Promise<{ posts: IBlogPost[] }> {
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        if (this.posts) {
          const previews = this.posts.map((post) => {
            return {
              title: post.title,
          ...
          reject({ error: "There was an error retrieving the posts." });
        }
      }, this.delay);
    });
  }
  ...
  allArchives() {
    ...
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        const archives = [];
        this.posts.sort(this.sortByDate);
  ...
  find(slug: string): Promise<IBlogPost> {
    return new Promise((resolve, reject) => {
      setTimeout(() => {
